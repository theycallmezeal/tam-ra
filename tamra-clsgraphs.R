...
neg_rel_part %>%
  ...
  geom_jitter()+
  facet_wrap(~ factor(FRAME, levels=c("NEG", "REL", "PART")), labeller=as_labeller(c(`NEG`="negation", `REL`="relativization", `PART`="participial")))+
  geom_smooth(
    method = "lm",
    data = neg_rel_part %>%
      filter(FRAME == "NEG", GENDER == "female")
  )+
  labs(x="age", y="preference for ra-", color="gender")
